...
clientID = vrep.simxStart('127.0.0.1', 19997, True, True, -500000,
...
if clientID != -1:
    ...
    # Some parameters:
    approachVector = [0, 0,
                      1]  # often a linear approach is required. This should also be part of the calculations when selecting an appropriate state for a given pose
    ...
    res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID, 'remoteApiCommandServer',
                                                                                 vrep.sim_scripttype_childscript,
                                                                                 'displayMessage', [], [], [
                                                                                     'Computing and executing several path planning tasks for a given goal pose.&&nSeveral goal states corresponding to the goal pose are tested.&&nFeasability of a linear approach is also tested. Collision detection is on.'],
                                                                                 emptyBuff,
                                                                                 vrep.simx_opmode_oneshot_wait)

    # Do the path planning here (between a start state and a goal pose, including a linear approach phase):
    inInts = [robotHandle, collisionChecking, minConfigsForIkPath, minConfigsForPathPlanningPath,
    ...
    if (res == 0) and len(path) > 0:
        ...
        res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID, 'remoteApiCommandServer',
                                                                                     vrep.sim_scripttype_childscript,
                                                                                     'removeLine', [line1Handle], [],
                                                                                     [], emptyBuff,
                                                                                     vrep.simx_opmode_oneshot_wait)
        res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID, 'remoteApiCommandServer',
                                                                                     vrep.sim_scripttype_childscript,
                                                                                     'removeLine', [line2Handle], [],
                                                                                     [], emptyBuff,
                                                                                     vrep.simx_opmode_oneshot_wait)
        ...
        res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID, 'remoteApiCommandServer',
                                                                                     vrep.sim_scripttype_childscript,
                                                                                     'displayMessage', [], [], [
                                                                                         'Computing and executing several path planning tasks for a given goal state. Collision detection is on.'],
                                                                                     emptyBuff,
                                                                                     vrep.simx_opmode_oneshot_wait)
        ...
        if (res == 0) and len(path) > 0:
            ...
            res, retInts, retFloats, retStrings, retBuffer = vrep.simxCallScriptFunction(clientID,
                                                                                         ...
                                                                                         vrep.sim_scripttype_childscript,
                                                                                         'displayMessage', [], [], [
                                                                                             'Computing and executing several linear paths, going through several waypoints. Collision detection is OFF.'],
                                                                                         emptyBuff,
                                                                                         vrep.simx_opmode_oneshot_wait)
